 ...
 * 
 * @brief Este archivo de cabecera contiene las definiciones del controlador
 * para utilizar el mo'dulo ESP8266 (utilizando comandos AT) con un 
 * microcontrolador PIC.
 * 
 * Este archivo contiene las declaraciones de las funciones para el controlador
 * que permite controlar un mo'dulo ESP8266 utilizando un microcontrolador PIC.
 * Con el objetivo de que el controlador sea facilmente migrable entre 
 * plataformas las funciones de bajo nivel estan declaradas como `extern` de 
 * forma tal que sea el usuario el encargado de implementarlas de acuerdo al 
 * compilador que utilize.
 * 
 * @date March 27, 2020, 4:51 PM
 */
...
extern "C" {
  ...
  /**
   * @brief Esta rutina inicializa el mo'dulo ESP8266
   * 
   * Esta rutina inicializa el mo'dulo ESP8266 para el modo wifi indicado.
   * 
   * @param[in] mode Permite establecer el modo de operacio'n del mo'dulo 
   * ESP8266. Ver \ref WIFI_MODE para mas detalles.
   * 
   * @return Un entero sin signo de 8 bits (`uint8_t`) indicando el resultado
   * de la operacio'n:
   * - 
   */
  ...
  /**
   * @brief Comprueba la comunciacio'n entre el MCU y el mo'dulo ESP8266.
   * 
   * Para comprobar la comunicacio'n envi'a el comando "AT" y comprueba que la
   * respuesta recibida sea "OK". Si no recibe respuesta o esta es diferente
   * retorn \b false.
   * 
   * @return
   *    - \b true: "OK" recibido
   *    - \b false: "OK" no recibido
   */
  bool ESP8266_CheckCommunication(void);                                        // implemented
  
  /**
   * @brief Permite obtener un string con el significado de un valor del
   * enum `ESP8266_RESPONSE` utilizado como para'metro.
   * 
   * @param[in] code Un valor del enum \ref ESP8266_RESPONSE.
   * 
   * @return Un `const char*` que apunta a un string con el significado del co'digo pasado
   * como para'metro.
   * 
   * \b Ejemplo
   * @code
  ...
  /**
   * @brief Reinicia el mo'dulo ESP8266.
   * 
   * Para reiniciar el mo'dulo se envi'a el comando AT+RST y se verifica la 
   * recepcio'n del mensaje 'ready'.
   * 
   * @return 
   *  - \b true: reiniciado con exito
   *  - \b false: respuesta no recibida
   */
  ...
  /**
   * @brief Permite establecer el modo de operacio'n del mo'dulo ESP8266 como:
   * Estacio'n, SoftAP o ambos simultaneamente.
   * 
   * @param[in] mode Modo de operacio'n. Ver enum \ref WIFI_MODE para ma's detalles.
   * @return 
   *    - ESP8266_RESPONSE_OK: Operacio'n exitosa;
...
//  uint8_t ESP8266_GetNetworkConnectionStatus(); // review this command: AT+CIPSTATUS
   ...
   */
  extern bool esp8266_ready2read(void);
   ...
   * @note Esta funcio'n tiene que ser implementada por el usuario.
   */
  extern char esp8266_getchar(void);
   ...
   * @note Esta funcio'n tiene que ser implementada por el usuario. 
   */
  extern void esp8266_putchar(char c);
